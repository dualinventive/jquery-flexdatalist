  ...
  return this.each( function( id ) {
    ...
    this.action = {
      ...
      keypressSearch: function( event ) {
        ...
        if ( !key || ( key !== 13 && ( key < 37 || key > 40 ) ) ) {
          _searchTimeout = setTimeout( function() {
            if ( ( options.minLength === 0 && length > 0 ) ||
              ( options.minLength > 0 && length >= options.minLength ) ) {
      ...
      redoSearchFocus: function( event ) {
        var val = _this.fvalue.get(),
          options = _this.options.get(),
          alias = $alias.val();
        if ( options.redoSearchOnFocus && ( ( alias.length > 0 && options.multiple ) ||
          ( alias.length > 0 && val.length === 0 ) ) ) {
      ...
      clearValue: function( event ) {
        ...
        if ( !options.multiple && options.selectionRequired && keyword.length <=
          options.minLength ) {
          _this.fvalue.clear();
        }
      },
    ...
    this.data = {
      ...
      remote: function( settings ) {
        ...
        var loadingMessage = _this.options.get( "loadingMessage" );
        $.ajax( $.extend(
          {
            type: options.requestType,
            dataType: "json",
            contentType: "application/" + options.requestContentType + "; charset=UTF-8",
            global: _this.options.get( "global" ),
            complete: function() {
              $this.removeClass( "flexdatalist-loading" );
            },
            beforeSend: function() {
              if ( loadingMessage ) {
                _this.results.remove( true );
                _this.results.empty( loadingMessage );
              }
            }
          }, settings, {
            success: function( data ) {
      ...
      extractRemoteData: function( data ) {
        var options = _this.options.get(),
          _data = _this.isDefined( data, options.resultsProperty ) ?
            data[ options.resultsProperty ] : data;
    ...
    this.search = {
      ...
      matches: function( item, keywords ) {
        ...
        if ( found.length === 0 ||
          ( options.searchByWord && found.length < ( keywords.length - 1 ) ) ) {
          return false;
        }
        return _item;
      },
      ...
      find: function( keyword, splitted ) {
        ...
        for ( var index = 0; index < splitted.length; index++ ) {
          ...
          if ( ( options.searchContain ? ( text.indexOf( keyword ) >= 0 ) :
      ...
      show: function( results ) {
        ...
        $li.on( "click", function( event ) {
          ...
          $( this ).addClass( "active" )
            .trigger( "active:flexdatalist.results", [ $( this ).data( "item" ) ] );
        }, function() {
      ...
      item: function( item ) {
        ...
        for ( var index = 0; index < visibleProperties.length; index++ ) {
          ...
          if ( visibleProperty.indexOf( "{" ) > -1 ) {
            ...
            if ( options.groupBy && options.groupBy === visibleProperty ||
              !_this.isDefined( item, visibleProperty ) ) {
              continue;
            }
            var $item = {};
    ...
    this.options = {
      ...
      _normalize: function( options ) {
        ...
        options.relatives =
          options.relatives && $( options.relatives ).length > 0 ? $( options.relatives ) : null;
        options.textProperty =
          options.textProperty === null ? options.searchIn[ 0 ] : options.textProperty;
    ...
    this.isDup = function( val ) {
      if ( !this.options.get( "allowDuplicateValues" ) ) {
        return _selectedValues.length > 0 && _selectedValues.indexOf( this.fvalue.value( val ) ) >
          -1;
      }
      return false;
    };
    ...
    this.csvToArray = function( value, _default ) {
      ...
      return typeof value === "string" ? value.split( _this.options.get( "valuesSeparator" ) ) :
        value;
    };
...
jQuery( function( $ ) {
  ...
  if ( !$document.data( "flexdatalist" ) ) {
    ...
    $( document ).mouseup( function( event ) {
      ...
      if ( ( !$target || !$target.is( ":focus" ) ) && !$container.is( event.target ) &&
      ...
      if ( keynum === 13 ) {
        ...
        if ( keynum === 40 ) {
          if ( index < length && $active.nextAll( ".item" ).first().length > 0 ) {
            $active =
              $active.removeClass( "active" ).nextAll( ".item" ).first().addClass( "active" );
          } else {
